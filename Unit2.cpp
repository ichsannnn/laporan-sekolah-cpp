...
void __fastcall TForm2::Button6Click(TObject *Sender)
{
	if(cari->Text=="")
		{
			MessageBoxA(0,"Masukan NIS Siswa yang akan dicari!","Warning",MB_ICONWARNING | MB_OK);
		}
	else
		{
			DM1->QuerySekolah->Close();
			DM1->QuerySekolah->SQL->Clear();
			DM1->QuerySekolah->SQL->Add("select * from siswa where nis='"+cari->Text+"'");
			DM1->QuerySekolah->Open();
				if(!DM1->QuerySekolah->IsEmpty())
					{
							Enis->Text=DM1->QuerySekolah->FieldByName("NIS")->AsString;
							Enisn->Text=DM1->QuerySekolah->FieldByName("NISN")->AsString;
							Enama->Text=DM1->QuerySekolah->FieldByName("Nama")->AsString;
							if(DM1->QuerySekolah->FieldByName("Gender")->AsString=="L")
								{
									RGgender->ItemIndex=0;
								}
							else if(DM1->QuerySekolah->FieldByName("Gender")->AsString=="P")
								{
									RGgender->ItemIndex=1;
								}
							Etempat->Text=DM1->QuerySekolah->FieldByName("TempatLahir")->AsString;
							Etanggal->Text=DM1->QuerySekolah->FieldByName("TanggalLahir")->AsString;
							Eagama->Text=DM1->QuerySekolah->FieldByName("Agama")->AsString;
							Eparents->Text=DM1->QuerySekolah->FieldByName("Parents")->AsString;
							Malamat->Lines->Text=DM1->QuerySekolah->FieldByName("Alamat")->AsString;
							Eijazah->Text=DM1->QuerySekolah->FieldByName("NoIjazah")->AsString;
							Etahun->Text=DM1->QuerySekolah->FieldByName("TahunIjazah")->AsString;
							Button1->Enabled=false;
							Button3->Enabled=false;
					}
				else if(DM1->QuerySekolah->IsEmpty())
					{
						MessageBoxA(0,"NIS tidak ada dalam daftar!","Warning",MB_ICONWARNING | MB_OK);
						cari->Clear();
					}
			cari->Enabled=false;
		}
...
void __fastcall TForm2::Button8Click(TObject *Sender)
{
	DM1->QuerySekolah->Close();
	DM1->QuerySekolah->SQL->Clear();
	DM1->QuerySekolah->SQL->Add("delete from siswa where nis = '"+cari->Text+"'");
	DM1->QuerySekolah->ExecSQL();
	MessageBoxA(0,"Data berhasil dihapus!","Information",MB_ICONINFORMATION | MB_OK);
	cari->Clear();
	Enis->Clear();
	Enisn->Clear();
	Enama->Clear();
...
{
	for(panelcari->Left=panelcari->Left;panelcari->Left<0;panelcari->Left++)panelcari->Left=panelcari->Left+10;
	{
...
{
	for(panelcari->Left=panelcari->Left;panelcari->Left>(-456);panelcari->Left--)panelcari->Left=panelcari->Left-10;
	{
...
void __fastcall TForm2::Button7Click(TObject *Sender)
{
	...
	DM1->QuerySekolah->Edit();
				...
				DM1->QuerySekolah->FieldByName("TahunIjazah")->AsString=Etahun->Text;
	DM1->QuerySekolah->Post();
	MessageBoxA(0,"Data berhasil diubah!","Information",MB_ICONINFORMATION | MB_OK);
	cari->Clear();
	Enis->Clear();
	Enisn->Clear();
	Enama->Clear();
...
void __fastcall TForm2::cariKeyDown(TObject *Sender, WORD &Key, TShiftState Shift)
